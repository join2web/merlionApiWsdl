...
class MerlionServiceGet extends MerlionWsdlClass
{
    ...
    /**
     * Method to call the operation originally named getItemsBarcodes
     * Documentation : Штрихкоды товаров (INT1178: метод для EAN)
     * @uses MerlionWsdlClass::getSoapClient()
     * @uses MerlionWsdlClass::setResult()
     * @uses MerlionWsdlClass::saveLastError()
     * @param MerlionStructGetItemsBarcodes $_merlionStructGetItemsBarcodes
     * @return MerlionStructGetItemsBarcodesResponse
     */
    public function getItemsBarcodes(MerlionStructGetItemsBarcodes $_merlionStructGetItemsBarcodes)
    {
        try
        {
            return $this->setResult(self::getSoapClient()->getItemsBarcodes($_merlionStructGetItemsBarcodes));
        }
        catch(SoapFault $soapFault)
        {
            return !$this->saveLastError(__METHOD__,$soapFault);
